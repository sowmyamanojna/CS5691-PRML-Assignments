...
\begin{itemize}
    \itemsep0em
    \item The model predictions are better when no regularization is applied. The $E_{rms}$ when regularization is applied is marginally higher than when $\lambda$ is 0.
    ...
    \item In case of no-regularization, we observed that the training, validation and testing $E_{rms}$ increased at the highest number of clusters considered. This couls be potentially due to improper initialization.
