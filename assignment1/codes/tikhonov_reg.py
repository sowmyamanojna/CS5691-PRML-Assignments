...
def gaussian_basis_fit(num_clusters, df_new, df_val_new, df_test_new, df_train, df_val, df_test, lmbda=0, fit_var="T_min"):
    ...
    for n_clu in num_clusters:
        ...
        if fit_var == "T_min":
            W1 = tikhonov(phi,mean_centers,var,lmbda) @ df_train["Next_Tmin"]
            W1 = W1.reshape(-1,1)
            ...
            plt.title("Next_Tmin; Clusters: "+str(n_clu) + ", $\lambda$: "+str(lmbda))
            plt.legend()
            name = "tikh_T_min_nclu_"+str(n_clu)+"_lambda_"+str(lmbda)+".png"
            plt.savefig(name)
            plt.show()
            ...
            W1 = tikhonov(phi,mean_centers,var,lmbda) @ df_train["Next_Tmax"]
            W1 = W1.reshape(-1,1)
            ...
            name = "tikh_T_max_nclu_"+str(n_clu)+"_lambda_"+str(lmbda)+".png"
            plt.savefig(name)
            plt.show()
