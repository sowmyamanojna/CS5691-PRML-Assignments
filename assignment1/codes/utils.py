...
def gaussian_basis_fit(num_clusters, df_new, df_val_new, df_train, df_val, lmbda=0, fit_var="T_min"):
    sse_list = []
    kmeans_list = []
    label_list = []
    val_label_list = []
    ...
    for n_clu in num_clusters:
        ...
        if fit_var == "T_min":
            ...
            plt.title("Next_Tmin; Clusters: "+str(n_clu))
            plt.legend()
            name = "images/t3_d3/T_min_nclu_"+str(n_clu)+".png"
            plt.savefig(name)
            plt.show()
            ...
            plt.title("Next_Tmax; Clusters: "+str(n_clu))
            plt.legend()
            name = "images/t3_d3/T_max_nclu_"+str(n_clu)+".png"
            plt.savefig(name)
            plt.show()

            error = np.linalg.norm(df_train["Next_Tmax"].to_numpy().reshape(-1,1)-pred)
            error_list.append(error)

            val_error = np.linalg.norm(df_val["Next_Tmax"].to_numpy().reshape(-1,1)-val_pred)
            val_error_list.append(val_error)


    return sse_list, error_list, val_error_list, kmeans_list, label_list, cluster_centers_list, val_label_list