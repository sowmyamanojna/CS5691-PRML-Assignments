	...
	def expectation(self):
		...
		for i in range(self.q):
			try:
				...
				self.gamma[:,i] = self.weights[i]*mvn.pdf(self.X, self.mu[i], self.C[i]+np.eye(self.C[i].shape[0])*1e-5)
				self.gamma = self.gamma/np.sum(self.gamma, axis=1).reshape(-1,1)
		...
		for i in range(n):
			...
			for j in range(self.q):
				try:
		   			...
		   			val += self.weights[j]*mvn.pdf(X_test[i], self.mu[j], self.C[j]+np.eye(self.C[j].shape[0])*1e-5)
			lk += np.log(val)
