...
class GMM():
    ...
    def fit(self, X, covariance_type="diag", tol=1e-5):
        """
        X: n*d
        mu: q*d
        C: q*d*d
...
class GMM_vl():
    ...
    def fit(self, X, epochs=100, covariance_type="diag", tol=1e-5):
        """
        X: n*d
        mu: q*d
        C: q*d*d
    ...
    def expectation(self):
        ...
        for i in range(self.q):
            try:
                ...
                self.gamma[:,i] = self.weights[i]*mvn.pdf(self.X, self.mu[i], self.C[i]+np.eye(self.C[i].shape[0])*1e-3)
                self.gamma = self.gamma/np.sum(self.gamma, axis=1).reshape(-1,1)
        ...
        for i in range(n):
            ...
            for j in range(self.q):
                try:
                    ...
                    val += self.weights[j]*mvn.pdf(X_test[i], self.mu[j], self.C[j]+np.eye(self.C[j].shape[0])*1e-3)
            lk += np.log(val)
    ...
    def probab(self, df):
        ...
        for key, item in grouped_df:
            ...
            X_select = selected_df.drop(["index", "image", "class"], axis=1).to_numpy()
            val = self.gaussian_val(X_select)
        print(val.shape)
